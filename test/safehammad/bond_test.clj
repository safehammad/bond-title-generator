...
(deftest pos->words-test
  (let [templates ["From/IN/ Russia/NN/ with/IN/ Love/NN/"
        ...
        output   {"IN" #{"from" "with"}
                  ...
                  "JJ" #{"gold" "thunder"}}]
    (is (= output (bond/pos->words templates)))))

(deftest match-capital-test
  (testing "capitalise"  ; First letter captalised and rest small
    (are [src-word dest-word] (= "Foo" (bond/match-capital src-word dest-word))
         "Big" "foo"
         "Big" "Foo"
         "Big" "foO"
         "BIg" "foo"))
...
(deftest remove-pos-test
  (are [result template] (= result (bond/remove-pos template))
...
(deftest replacement-words-test
  (let [templates ["From/IN/ Russia/NN/ with/IN/ Love/NN/" "Gold/JJ/finger/NN/"]]
    (are [result word pos] (= result (set (bond/replacement-words templates word pos)))
...
(deftest find-unknown-genres-test
  (are [genres result] (= result (bond/find-unknown-genres genres))
       ["jamesbond" "foo"] ["foo"]
